...
```{r OV/BV and serum level}
OV_BV <-read_csv("~/Dropbox/Conference/BIOSTAT2017/Data.Reproducible/Data/priemelDataReconstruction.csv")
ggplot(OV_BV, aes(x=`OV/BV`, y=SerumLevelInNgPerMl)) + geom_point() + geom_smooth(method = "auto")
```
...
library(dplyr)
head(OV_BV)
OV_BV_2 <- OV_BV %>% filter(!as.numeric(OV_BV$`OV/BV`)>2)
head(OV_BV_2)
ggplot(OV_BV_2, aes(x=`OV/BV`, y=SerumLevelInNgPerMl)) + geom_point() + geom_smooth(method = "auto")
OV_BV_3 <- OV_BV_2 %>% filter(!SerumLevelInNgPerMl>10)
head(OV_BV_3)
ggplot(OV_BV_3, aes(x=`OV/BV`, y=SerumLevelInNgPerMl)) + geom_point() + geom_smooth(method = "auto")
```
